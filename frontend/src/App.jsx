...
function App() {
  ...
  const [isLoginView, setIsLoginView] = useState(true), [authEmail, setAuthEmail] = useState(''), [authPassword, setAuthPassword] = useState(''), [authName, setAuthName] = useState('');
  ...
  const handleLogin = async (e) => {
    ...
    try { const res = await axios.post(`${AUTH_API}/login`, { email: authEmail, password: authPassword }); const { token, user } = res.data; setToken(token); setUser(user); localStorage.setItem('token', token); localStorage.setItem('user', JSON.stringify(user)); axios.defaults.headers.common['Authorization'] = `Bearer ${token}`; setAuthEmail(''); setAuthPassword(''); }
    catch (err) { alert(err.response?.data?.error || 'Login gagal'); } finally { setSubmitting(false); }
  };

  const handleRegister = async (e) => {
    e.preventDefault(); setSubmitting(true);
    try { await axios.post(`${AUTH_API}/register`, { email: authEmail, password: authPassword, name: authName }); alert('Registrasi berhasil! Silakan login.'); setIsLoginView(true); setAuthName(''); }
    catch (err) { alert(err.response?.data?.error || 'Registrasi gagal'); } finally { setSubmitting(false); }
  };
  ...
  const handleUpdateNote = async () => {
    ...
    catch (err) { console.error(err); alert('Gagal memperbarui catatan.'); } finally { setSubmitting(false); }
  };
          ...
          <div className="flex-1 flex items-center justify-center">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass w-full max-w-md p-8 sm:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden">
              ...
              <form onSubmit={isLoginView ? handleLogin : handleRegister} className="space-y-5">
                {!isLoginView && <div className="space-y-2"><label className="text-[10px] font-bold text-muted/50 uppercase tracking-widest ml-1">Nama Lengkap</label><div className="relative"><UserIcon className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40" size={18} /><input type="text" value={authName} onChange={(e) => setAuthName(e.target.value)} className="w-full bg-border/5 border border-border/20 rounded-2xl pl-12 pr-4 py-3 focus:outline-none focus:border-accent/50 text-foreground transition-all" placeholder="Rifqi Fadil" /></div></div>}
                <div className="space-y-2"><label className="text-[10px] font-bold text-muted/50 uppercase tracking-widest ml-1">Email Address</label><div className="relative"><Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40" size={18} /><input type="email" required value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} className="w-full bg-border/5 border border-border/20 rounded-2xl pl-12 pr-4 py-3 focus:outline-none focus:border-accent/50 text-foreground transition-all" placeholder="name@example.com" /></div></div>
                <div className="space-y-2"><label className="text-[10px] font-bold text-muted/50 uppercase tracking-widest ml-1">Password</label><div className="relative"><Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-muted/40" size={18} /><input type="password" required value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} className="w-full bg-border/5 border border-border/20 rounded-2xl pl-12 pr-4 py-3 focus:outline-none focus:border-accent/50 text-foreground transition-all" placeholder="••••••••" /></div></div>
          ...
          <>
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-12">
              ...
              <div className="flex items-center gap-2 bg-glass p-1.5 rounded-2xl border border-border w-full md:w-auto justify-center shadow-xl">
                <button onClick={() => setActiveTab('todos')} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2 rounded-xl font-semibold transition-all ${activeTab === 'todos' ? 'bg-accent text-white shadow-lg' : 'text-muted hover:text-foreground'}`}><ListTodo size={18} /> <span>Task</span></button>
                <button onClick={() => setActiveTab('notes')} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2 rounded-xl font-semibold transition-all ${activeTab === 'notes' ? 'bg-accent text-white shadow-lg' : 'text-muted hover:text-foreground'}`}><FileText size={18} /> <span>Notes</span></button>
                ...
                {activeTab === 'todos' ? (
                  <motion.div key="todos" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex-1">
                    ...
                    <div className="space-y-4 sm:space-y-5">
                      {todos.map((todo) => (
                        <motion.div key={todo.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => openDetail(todo, 'todo')} className={`glass p-5 sm:p-6 rounded-[2rem] border border-border flex flex-col sm:flex-row items-center gap-4 sm:gap-6 group cursor-pointer ${todo.completed ? 'opacity-50 grayscale' : 'hover:shadow-xl hover:shadow-accent/5'}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <h3 className={`text-xl font-extrabold tracking-tight truncate ${todo.completed ? 'line-through text-muted font-normal' : ''}`}>{todo.task}</h3>
                            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2">
                              <span className="flex items-center gap-2 text-sm font-medium text-muted/80"><BookOpen size={16} className="text-accent/60" /> {todo.course}</span>
                              <span className="flex items-center gap-2 text-sm font-semibold text-accent"><Calendar size={16} /> {new Date(todo.deadline).toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                              {!todo.completed && <span className="flex items-center gap-2 text-xs font-bold uppercase tracking-tighter text-accent/60"><Clock size={14} /> {getCountdown(todo.deadline)}</span>}
                            </div>
                          </div>
                  ...
                  <motion.div key="notes" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                      {notes.map((note) => (
                        <motion.div key={note.id} layout onClick={() => openDetail(note, 'note')} className="glass p-6 sm:p-8 rounded-[2.5rem] border border-border group relative flex flex-col justify-between cursor-pointer hover:shadow-xl hover:shadow-accent/5 transition-all">
                          <div><h3 className="text-xl font-extrabold tracking-tight mb-4 pr-10">{note.title}</h3><p className="text-muted leading-relaxed font-medium text-sm sm:text-base line-clamp-4 whitespace-pre-wrap">{note.content}</p></div>
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} onClick={(e) => e.stopPropagation()} className="glass w-full max-w-lg rounded-[2.5rem] p-8 sm:p-10 shadow-2xl relative overflow-hidden max-h-[90vh] overflow-y-auto">
              ...
              {itemType === 'todo' ? (
                <div className="space-y-6 pt-4">
                  <span className="inline-flex px-3 py-1 rounded-full bg-accent/10 text-accent text-[10px] font-bold uppercase tracking-widest w-fit">Task Details</span><h2 className="text-3xl font-extrabold tracking-tight">{selectedItem.task}</h2>
                  <div className="space-y-4 pt-4 border-t border-border/40 flex flex-col">
                    <ModalInfo icon={<BookOpen size={20} />} label="Course" value={selectedItem.course} />
                    <ModalInfo icon={<Calendar size={20} />} label="Deadline" value={new Date(selectedItem.deadline).toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })} />
                    {!selectedItem.completed && <ModalInfo icon={<Clock size={20} />} label="Countdown" value={getCountdown(selectedItem.deadline)} color="text-accent" />}
                    <ModalInfo icon={selectedItem.completed ? <CheckCircle size={20} /> : <Circle size={20} />} label="Status" value={selectedItem.completed ? 'Completed' : 'Pending'} color={selectedItem.completed ? 'text-green-500' : 'text-amber-500'} />
                  </div>
                </div>
              ) : (
                <div className="space-y-6 pt-4">
                  <div className="flex justify-between items-center mb-4">
                    <span className="px-4 py-1.5 rounded-full bg-accent/10 text-accent text-[11px] font-bold uppercase tracking-widest">{isEditing ? 'EDITING NOTE' : 'NOTE DETAILS'}</span>
                    {!isEditing && <button onClick={startEditing} className="px-4 py-2 rounded-xl border border-accent/20 bg-accent/5 text-accent font-bold text-xs flex items-center gap-2 hover:bg-accent hover:text-white transition-all"><Edit3 size={16} /> EDIT</button>}
                  </div>
                  {isEditing ? (
                    <div className="space-y-4 pt-4"><input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="w-full bg-border/5 border border-border/20 rounded-xl px-4 py-2.5 text-xl font-bold focus:outline-none focus:border-accent/40 transition-all" /><textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full bg-border/5 border border-border/20 rounded-xl px-4 py-4 min-h-[200px] resize-none focus:outline-none focus:border-accent/40 transition-all" /><button onClick={handleUpdateNote} className="w-full bg-accent text-white py-3 rounded-2xl font-bold shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 transition-all">{submitting ? <Loader2 className="animate-spin" size={20} /> : <Save size={22} />} UPDATE NOTE</button></div>
                  ) : (
                    <><h2 className="text-4xl font-black tracking-tight leading-none mb-6 uppercase">{selectedItem.title}</h2><div className="max-h-[40vh] overflow-y-auto mb-10"><p className="text-muted leading-relaxed font-semibold text-xl italic whitespace-pre-wrap">"{selectedItem.content}"</p></div>
                    <div className="pt-8 border-t border-border/40 flex justify-between items-start">
                      <div className="space-y-1"><p className="text-[10px] font-black text-muted/50 uppercase tracking-widest">CREATED AT</p><p className="text-sm font-bold">{new Date(selectedItem.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p></div>
                      <div className="text-right space-y-1"><p className="text-[10px] font-black text-muted/50 uppercase tracking-widest">LAST UPDATED</p><p className="text-sm font-bold">{new Date(selectedItem.updatedAt || selectedItem.createdAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p></div>
                    </div></>
                  )}
                </div>
              )}
